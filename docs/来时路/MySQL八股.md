...
### 两条UPDATE语句处理一张表的不同的主键范围的记录，一个<10一个>15，会不会遇到阻塞，为什么？
不会，因为锁住的范围不一样，不会形成冲突。
- 第一条UPDATE SQL（id<10）在可重复读隔离级别下会以next-key锁的形式锁住id<10的范围。
- 第二条UPDATE SQL（id>15）在可重复读隔离级别下会以next-key锁的形式锁住id>15的范围。
