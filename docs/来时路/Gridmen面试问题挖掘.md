...
## 模版化组件分发
### TemplateRegistry和ViewRegistry解决了什么问题？
本质上TemplateRegistry + ViewRegistry 是一个二维能力分发机制，解决的问题是，系统中存在着多种资源类型，而对于一种资源来说其视图方式也存在着多种类型，如果直接写条件来计算应挂载的组件，那么需要写出大量重复的条件运算或是switch-case双层嵌套结构代码，对于一个资源来说要添加的分枝数就是：
```txt
增改查三种行为 ✖️ 视图数
```
而基于TemplateRegistry和ViewRegistry的注册表把这种组合关系集中成配置映射。它让顶层容器不必关注具体的挂载组件细节，而是挂载通用的操作行为组件即可。通过传入的ResourceNode的template属性和右键菜单的点击事件，两种属性通过映射即可得到相应资源的相应操作组件。

总体来说，他们分别回答这样的问题：
- TemplateRegistry：这个资源是什么，它有哪些领域的行为
- ViewRegistry：这种资源在这种视图中，以哪种模式呈现

#### TemplateRegistry
本质上是资源类型注册表，其结构是一个对象，为防止取到未注册的资源类型，这里通过proxy对注册表的取值操作进行拦截，当取值为undefined时返回默认资源模版，即系统说明。防止出现后端注册新资源类型，而前端对应组件未及时更新对齐导致整棵资源树报错崩溃的情况。

#### ViewRegistry
本质上是视图与业务组件注册表，在ViewRegistry上绑定的viewModels是组件工厂集合。这里最大的用处是为不同资源的业务组件注入统一的同一个mapbox地图上下文。

如果直接调用组件工厂函数并传入mapbox上下文，根据引用类型比较，即使输入相同，函数输出的结果对象的引用也是不同的，React据此会认为是旧组件的销毁和新组件的挂载，如果useEffect的回调中涉及地图图层销毁、那么地图将重新渲染，所以这里通过useMemo包裹地图上下文，并通过工厂将统一地图context传给所有业务组件。

#### 这样做的目的
- 将资源类型与行为模板解耦。
- 建立视图、模板与 create/check/edit 状态之间的映射。
- 新增资源类型时减少核心框架中的条件分支，提高系统后续的可扩展性。
- 未知模板回退到默认模板，提高兼容性。
