...
### MVC、MVP、MVVM
他们三个都有Model和View层。
- Model为模型层，主要管理业务模型的数据和行为
- View为展示层，主要负责管理用户界面

这三个架构模式都是为了解耦Model和View，不同点在于实现解耦的方式不同，分别采用了Controller、Presenter、ViewModel来实现解耦。
#### MVP
即Model-View-Presenter。
- Model：模型层，主要负责数据的获取、存储和处理
- View：视图层，主要负责数据的展示，并响应用户操作
- Presenter：表示层，主要负责处理业务逻辑，将Model和View进行连接。Presenter会从Model中获取数据，并将数据传递给View进行展示，同时也会接收View的用户操作，并将操作传递给Model进行处理。

MVP中View是哑视图，View只负责展示数据和接收用户操作，不包含任何业务逻辑。Presenter是中介者，负责处理业务逻辑，将Model和View进行连接。Presenter会从Model中获取数据，并将数据传递给View进行展示，同时也会接收View的用户操作，并将操作传递给Model进行处理。

#### MVC
即Model-View-Controller
- Model：模型层，用于封装应用程序的业务逻辑相关的数据以及对数据的处理方法，一旦数据发生变化，模型将通知有关的视图
- View：视图层，主要负责数据的展示，并响应用户操作
- Controller：控制层是模型层和视图层之间的纽带，接收View层传来的用户事件传递给Model层，将Model传出的数据结果传递给View层以更新。

MVC 的数据同步是命令式的、需要手动实现：用户操作 View，Controller 收到输入后调用 Model 的方法修改数据；Model 变更后通过观察者模式通知 View；View 因为认识 Model、订阅了 Model 的变化，收到通知后自己读取数据并重绘。Controller 只负责把用户输入翻译成 Model 操作，不监听 Model、也不直接刷新 View。相比 MVP 的"Presenter 显式调用 View 方法更新"和 MVVM 的"绑定自动同步"，MVC 的 View 与 Model 耦合较高、逻辑散在 View 里，也因此最难脱离 DOM 单独测试。

#### MVVM
即Model-View-ViewModel。
- Model：模型层，Model只关心数据本身，不关心任何行为，他类似一个数据仓库
- View：视图层，通过模版语言来声明式地将数据渲染进DOM，当ViewModel对Model更新时，会通过数据绑定的方式将数据更新到View层
- ViewModel：类似于Presenter，ViewModel会对View层的声明进行处理，当ViewModel中数据变化，View层会进行更新。如果是双向绑定,一旦View对绑定的数据进行操作，则ViewModel中的数据也会进行自动更新

MVVM的数据同步是声明式的，由框架自动处理。View只声明绑定了ViewModel的哪个属性，剩下的数据变更事件由绑定机制自动处理。ViewModel甚至不知道View的存在，可以脱离View做单独测试。

有点：
- 低耦合：View可以独立于Model变化和修改，一个ViewModel可以绑定到不同的View上，当View变化时Model可以不变，当Model变化时可以不影响View
- 可重用性：可以把一些视图逻辑放在ViewModel中，ViewModel可以被多个View复用
- 独立开发：开发人员可以专注于业务逻辑和数据的开发，设计人员可以专注于页面XML代码的编写
- 可测试：由于ViewModel与View的解耦，使得单元测试变得更加容易

#### React和Vue算是MVVM吗？
Vue是MVVM框架，React是V层框架。

Vue的双向数据绑定是通过数据劫持和发布订阅模式实现的，数据变化时会通知视图更新，视图变化时也会通知数据更新。视图只是声明绑定了什么数据，剩下的由框架自动处理。

React的设计理念是组件化，组件就是一个个独立的模块，组件之间通过props和state进行数据传递和状态管理。React的核心思想是单向数据流，即数据从父组件流向子组件，子组件通过props接收父组件传递的数据，并通过回调函数将数据传递回父组件。React的状态管理主要依赖于state和setState方法，当state发生变化时，React会重新渲染组件，从而实现数据的更新。

## 地信相关
### 业内常用的算法和数据结构有哪些？
数据结构：
- R树：空间数据库索引
- 四叉树：二维栅格编码、地图瓦片、区域检索
- 八叉树：三维点云、体数据

算法：
- 空间差值：IDW反距离加权法、克里金差值
- 重采样：最近邻、双线性、双三次
- 三角网生成法
- 道格拉斯-普克算法：矢量抽稀
- Dijkstra、A*、Prim
...
演示Demo点击查看：<a href="https://blog-1385521233.cos.ap-guangzhou.myqcloud.com/docs/job/sticky和fixed.html" target="_blank" rel="noopener noreferrer">sticky与fixed的区别</a>
#### fixed
fixed基于视口定位，元素脱离文档流，不占位，当滚动式从头到尾保持在视口固定位置，全页面生效。
...
#### sticky
sticky基于滚动容器定位，元素不脱离文档流，保留占位，需要设置触发阈值和滚动空间，在触发前先随文档流滚动，到达阈值才被吸住，其有效范围不能超出父容器。
...
### SVG和Canvas的区别
#### SVG
- 每个图形元素都是一个DOM节点，描述图形本身的路径、形状。
- 保留图形的矢量信息，放大缩小不会失真。
- 由浏览器渲染引擎绘制
- 因为每个元素都是DOM，天然支持事件绑定
- 改某个元素属性即可局部刷行
#### Canvas
- 是一个整体的位图画布，描述像素结果。
- 只保留最终像素信息。
- 由脚本逐像素绘制
- 整块画布是一个元素，需要自己计算坐标来判断事件
- 修改需要重绘整个画布，无法局部刷新

### 如何触发GPU加速
在HTML解析过程中，对于Render Tree的合成主要是CPU完成，最后阶段的合成是GPU完成，GPU加速就是指把某个元素提升到一个独立的合成层，之后它的动画只发生在最后一步——由 GPU 把各层"合"成一张画面。前面昂贵的 Layout/Paint 都不用重做。动画期间浏览器每帧只做一次合成（composite），这就是性能的来源。
...
- 浏览器自动提升：
    - position: fixed
    - `<video>`、`<canvas>`、`<iframe>`、`<embed>`、`<object>`等标签
    - filter、backdrop-filter、clip-path、mask-image等属性

理想实践是用transform和opacity来做动画，避免使用top、left、width、height等属性，因为这些属性会触发回流重绘，影响性能。
...
#### 寄生组合式继承
首先子构造函数内调用父构造函数，以继承父构造函数的属性，然后将子构造函数的prototype指向父构造函数的prototype通过Object.create()创建的对象完成原型链设置，再将子构造函数的prototype的constructor指向子构造函数本身，这样就实现了寄生组合式继承。
...
- 引用计数：一张引用表记录每个对象的引用次数，当引用次数为0时，说明该对象不再被使用，可以被回收。但是循环引用会导致永远无法回收。c
...
### Number(5)与New Number(5)的区别
- Number(5)：返回一个基本数据类型的数值5
- new Number(5)：返回一个Number包装对象，包装了基本数据类型的数值
```js
...
new Number(5) == 5 // true
```
几乎不使用new Number()这样的包装对象，因为容易造成比较陷阱和真值混淆。

### 包装对象
JS中的Boolean、Number、String都具有双重身份，他们既可以当普通函数用，也可以通过new的方式作为构造函数使用，后者以构造函数使用的方式会产生包装对象——一个包装了原始值的对象。
```js
typeof 'abc' // "string"
...
#### 为什么存在——原始值可以调用方法
原始值本身没有属性和方法，但是却可以实现'abc'.toUpperCase()，这是因为JS在调用方法时，会自动将原始值包装成对应的包装对象，然后调用方法，最后再将结果转换回原始值。
1. 引擎遇到在原始值上访问属性或方法的情况
2. 临时创建一个包装对象
3. 在临时对象上执行操作
4. 用完后立刻丢弃，原始值不受影响

#### ToPrimitive拆箱规则
ToPrimitive是JavaScript中的一种内部操作，用于将对象转换为原始值。它遵循以下规则：
- 如果对象参与+、==、!=等运算符，先尝试valueOf()方法，不是原始值再尝试toString()方法
- 模版字符串，先尝试toString()方法，再尝试valueOf()方法

#### 为什么[] == ![]
1. 拆右边，对于![]，!具有高运算优先级，先调用ToBoolean([])得到true，再取反得到false
2. ==一侧出现Boolean，将false转为数字0，此时是[] == 0
3. 此时一侧是Object、一侧是Number，调用ToPrimitive([])先valueOf()得到[]本身，再调用toString()得到''，此时是'' == 0
4. 此时一侧是String、一侧是Number，调用ToNumber('')得到0，此时是0 == 0
5. 最终返回true
...
### JavaScript加载与执行的过程
HTML 解析 → JS 下载 → JS 编译（提升）→ JS 执行 → 异步任务进入事件循环。
#### HTML解析阶段
首先是浏览器解析HTML文档，遇到script标签时，浏览器会暂停HTML解析，开始下载并执行脚本
- 无defer无async：立即下载并执行，阻塞HTML解析
- 有defer：异步下载，等HTML解析完成后再执行，按顺序执行
- 有async：异步下载，下载完成后立即执行，可能打断HTML解析

#### 预加载
浏览器在解析HTML的同时，会启动预解析器，并行下载外部资源（JS/CSS/图片等），提升性能

#### 编译阶段
在执行阶段之前，JavaScript引擎会对代码进行预编译：
- 变量提升：var声明会提升到作用域顶部，但值不会提升
- 函数提升：函数声明会整体提升，可以在声明前调用

同时创建执行上下文，包括变量环境、词法环境、作用域链等

#### 执行阶段
按顺序执行代码，执行过程中变量会赋值，函数被调用

#### 事件循环
遇到异步任务，会交给浏览器或Node的任务队列处理。主线程执行完同步任务后，从队列取出异步任务回调再执行。

### JS编译的大致流程
1. 词法分析：将源码解析为token流，将字符切分为最小的有意义单元
2. 语法分析：把token转换为AST抽象语法树，这个过程会做作用域提升，并确定let和const的TDZ
3. 预解析：跳过函数体，只解析顶层，只有函数等第一次被执行时才解析
4. 字节码生成：将AST转换为字节码，这是一种紧凑的中间标识，解释器可以跑，优化编译器也可以跑
5. 解释执行：将字节码转换为执行结果，每执行一条就记录“运行时类型反馈”
6. 优化编译：字节码和反馈转换为机器码，将反馈里的类型假设烘焙进机器码
7. 反优化：将优化后的机器码还原为字节码，出现在类型假设不成立的情况下


### a怎么赋值的情况下`if(a == 1 && a== 2 && a == 3)`判断为true
对于`==`运算符，JS会进行类型转换，进行toPrimitive操作，最终将对象转换为原始值，那么会对a去调用valueOf()和toString()方法，返回不同的值，从而使得条件成立。

所以可以对a的valueOf()或toString()方法进行重写，使得每次调用返回不同的值。
```js
let a = {
  i: 1,
  valueOf() {
...
#### 事件池是什么，为什么React 17后取消了？
先前React为了可以复用 SyntheticEvent(抹平浏览器差异、统一接口)对象搞了个池(pool)，也就是事件池，为了减少事件对象的重复创建，降低GC压力。集体机制是：
- 事件触发时，React从池中取一个合成事件SyntheticEvent，并把target和type等属性填入。
- 派发给对应的handler，这棵树的handler同步跑完
- handler结束后，React将事件对象归还到池中复用，做法是将属性全部置空。

但这样造成的问题是，只能在handler同步执行的窗口内读事件属性，异步读取时属性已清空。

React 17取消了事件池，是因为：
1. 现代浏览器/JS 引擎对象分配和 GC 已经够快,池化最初想省的内存收益不再成立;
2. 属性清空这套"重置/归还"逻辑本身有成本;
3. 造成大量异步读 event 的 bug,而收益撑不起这份困惑。
...
JSX作为React的语法糖，最终会通过Babel编译成JS代码，在React16及之前是通过对React.createElment()的调用，这个方法会返回一个虚拟DOM JS对象，本质上JSX是为了简化对React.createElment()的调用。React.createElement()接收三个参数：
- type：元素的类型字符串，首字母小写为HTML标签，首字母大写为React组件
- props：元素的属性对象，包含了元素的所有属性和事件监听器
- children：元素的子节点，可以是字符串、数字、React元素或数组

而从React 17开始，编译期不再默认编译成React.createElement()，而是编译成jsx-runtime中的jsx，它将children也作为props的一部分传入，接收两个参数：
- type：元素的类型字符串
- props：元素的属性对象，其中也包含children。
...
在React中setState“总是”异步的，修改结果并不会立刻反映到state中，不过可以使用flushSync()方法来强制同步更新state。setState只是触发了更新事件enqueue进入fiber的updateQueue以及ensureRootIsScheduled来请求调度一次渲染，多次的setState只是通过首个setState利用ensureRootIsScheduled通过messageChannel安排了一个render宏任务进入事件循环队列，不过所有setState都确保了自己的update进入updateQueue，然后根据他们的lane优先级由Scheduler来安排执行时机，同级lane的update在同一次render中完成批处理，setState行为其对state修改的真正执行时机在React的调和阶段，通过遍历updateQueue中的update将他们作用于baseState计算出修改后的新的state，最终在commit阶段的mutation子阶段根据fiber上的subtreeFlags以及flags对真实DOM一次性完成同步且不可中断的增删改更新。也就是说触发和执行并不是同步的。
...
3. `<Router>`监听到URL变化，更新location对象，通过Context或useSyncExternalStore通知组件树重渲染
...
#### Context
Context是React原生提供的全局状态管理方案，使用时通过Context.Provider包裹组件树，在消费组件端用useContext()来获取Context的值。Context适合用于跨组件传递一些全局状态，如主题、语言、用户信息等，适合中小型项目或简单的状态管理需求。React 19后不用再写Provider了，直接在消费组件中useContext()即可。

优点是：
- 简单易用：原生hook，不需要额外的库，使用起来非常简单，适合中小型项目
- 轻量级：不需要引入额外的依赖，减少了项目的体积和复杂度

缺点是：
- 性能问题：Context的值变化会导致所有消费该Context的组件重新渲染，可能会引起性能问题，尤其是在大型应用中
- 不适合复杂状态管理：Context适合用于传递一些简单的全局状态，对于复杂的状态管理需求，Context可能不够灵活和强大

#### Redux
Redux是React生态中最流行的状态管理库，它通过单例store来管理应用全局状态，使用时通过Provider包裹组件树，在消费组件端用useSelector()来获取store的状态，用useDispatch()来派发action。Redux适合用于大型应用或复杂的状态管理需求，如需要在多个组件之间共享状态、需要对状态进行复杂的操作和计算等。

优点是：
- 可预测性：Redux的状态是不可变的，通过纯函数reducer来更新状态，保证了状态的可预测性和可追踪性
- 中间件支持：Redux提供了中间件机制，可以在action被派发到reducer之前进行预处理，如日志记录、异步操作等，增强了Redux的功能和灵活性
- 开发者工具支持：Redux有强大的开发者工具，可以方便地调试和追踪状态的变化，提升了开发效率

缺点是：
- 学习成本高：Redux的概念和使用方式相对复杂，需要学习action、reducer、store等概念，对于初学者来说可能有一定的学习成本
- 样板代码多：Redux的使用需要编写大量的样板代码，如action、reducer、store的配置等，增加了开发的复杂度和工作量

不过现在已经有了RTK（Redux Toolkit），它是Redux官方推荐的工具库，提供了简化的API和最佳实践，减少了样板代码，提高了开发效率。

### useSyncExternalStore是如何工作的？
useSyncExternalStore是React 18引入的新hook，它用于在组件中订阅外部store的状态，并在store状态变化时驱动React执行Render。它的主要作用是解决在React中使用外部store时可能出现的状态撕裂tearing，确保组件在渲染时能够获取到最新的store状态且保持一致。

```tsx
const snapshot = useSyncExternalStore(
    subscribe,          // 订阅外部store
    getSnapshot,        // 获取当前客户端状态
    getServerSnapshot   // 获取当前服务端状态
);
```
核心机制是：React通过getSnapshot读取外部状态，通过subscribe注册更新通知；外部状态变化后，React重新读取快照，并用Object.is()进行前后结果浅比较，决定是否触发组件重新渲染。
```txt
外部 Store
   │
   ├── getSnapshot ──→ React 读取状态
   │
   └── subscribe ────→ React 接收变化通知
                           │
                           ↓
                    比较前后快照
                           │
                    不同才重新渲染
```
useSyncExternalStore只负责监听外部store的状态变化并驱动React重新渲染，它不负责store的状态管理和更新，store的状态管理和更新需要由外部store本身来实现。
...
- 闲置阶段（idel，prepare）：仅系统内部使用，Node.js内部使用该阶段来执行一些准备工作
