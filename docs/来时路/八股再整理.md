...
plugin是一种遵循一定规范的应用程序接口编写出来的程序，只能运行在程序规定的系统下，因为其需要调用原纯净系统提供的函数库或者数据，在Webpack中也一样，plugin赋予了Webpack各种灵活的功能，如打包优化、资源管理、环境变量注入等，他们运行在Webpack的不同阶段，贯穿Webpack的生命周期。
