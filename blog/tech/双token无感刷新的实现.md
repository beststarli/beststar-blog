...
前几天一个深夜的一点半，困意终于逼自己关上手机准备入睡，突然一个念头闪过，才想起现在做的系统其实还没有真正引入用户的概念，当然也就没做用户登录功能，之前经常听劳达还有橙子哥提起双token无感刷新，感觉有点意思，我想在这个系统里实现一下，带着这个想法睡着了。
...
## 前端搭建
### 界面组件
前端界面的搭建流程还是老样子，我习惯先给v0写一串十分详细的需求和提示词让它帮忙生成一下界面样式，然后拿它生成的第一个version的界面代码迁移到自己的项目里再根据需要进行修改。
...
![注册页](/img/tech/doubleToken/zhuce.png)

![重置页](/img/tech/doubleToken/chongzhi.png)

可以看到有些文本的间距看着不太舒服，简单调整了一下margin和gap属性，v0不习惯给按钮添加cursor-pointer，为了交互感我比较喜欢按钮悬浮的手势变化就自己再添加了一下。

对于注册页来说，这个验证系统的用户只需要拥有邮箱和密码即可，因此删去了用户名输入组件。

对于密码重置页来说，这个Demo系统暂时先不考虑实现真正的密码重置功能。

仔细审查v0提供的注册页和密码重置页的前端代码，发现在**返回登录**这个按钮逻辑的路由跳转上它还是使用了Next.js中的Link组件来实现的，由于我没有对Next.js的公共变量进行配置，导致发生了以下报错：
